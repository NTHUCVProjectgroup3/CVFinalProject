...
def SinGAN_generate(Gs,Zs,reals,NoiseAmp,opt,dir2save=None, in_s=None,scale_v=1,scale_h=1,n=0,gen_start_scale=0,num_samples=50):
    ...
    for G,Z_opt,noise_amp in zip(Gs,Zs,NoiseAmp):
        ...
        for i in range(0,num_samples,1):
            ...
            if n == len(reals)-1:
                if opt.mode == 'train':
                    dir2save = '%s/RandomSamples/%s/gen_start_scale=%d' % (opt.out, opt.input_name[:-4], gen_start_scale) if dir2save is None else dir2save
                else:
                    dir2save = functions.generate_dir2save(opt) if dir2save is None else dir2save
                try:

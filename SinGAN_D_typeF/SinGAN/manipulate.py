...
def SinGAN_generate_V2(Gs,Zs,reals,segs,NoiseAmp,opt,in_s=None,scale_v=1,scale_h=1,n=0,gen_start_scale=0,num_samples=50):
    #if torch.is_tensor(in_s) == False:
    if in_s is None:
        in_s = torch.full(reals[0].shape, 0, device=opt.device)
    images_cur = []
...
def SinGAN_generate_V3(Gs,Zs,segs,NoiseAmp,opt,in_s=None,scale_v=1,scale_h=1,n=0,gen_start_scale=0,num_samples=50):
    if in_s is None:
        in_s = torch.full(segs[0].shape, 0, device=opt.device)
    ...
    for G,Z_opt,noise_amp in zip(Gs,Zs,NoiseAmp):
        ...
        for i in range(0,num_samples,1):
            ...
            if images_prev == []:
                ...
                if opt.mode != "SR":
                    I_prev = I_prev[:, :, 0:round(scale_v * segs[n].shape[2]), 0:round(scale_h * segs[n].shape[3])]
                    I_prev = m(I_prev)
                    I_prev = I_prev[:,:,0:z_curr.shape[2],0:z_curr.shape[3]]
                    I_prev = functions.upsampling(I_prev,z_curr.shape[2],z_curr.shape[3])
                else:

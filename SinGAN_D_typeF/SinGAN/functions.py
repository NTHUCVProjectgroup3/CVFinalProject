...
def creat_segment_pyramid(seg, segs, opt):
    seg = seg[:, 0:3, :, :]
    h, w = seg.shape[2], seg.shape[3]
    for i in range(0, opt.stop_scale + 1, 1):
        scale = math.pow(opt.scale_factor, opt.stop_scale - i)
        curr_seg = resize(seg.cpu().numpy(), (1, 3, np.ceil(h * scale), np.ceil(w * scale)), order=0)
...
def generate_dir2save(opt):
    ...
    elif (opt.mode == 'seg_train'):
        dir2save = 'TrainedModels/%s/scale_factor=%f_seg' % (opt.input_name[:-4], opt.scale_factor_init)
